...
impl DesktopComposition {
    ...
    /// Build a `ShellWorkContent` snapshot from the current composition state.
    ///
    /// All workspace-surface content (editor body, tabs, breadcrumb, explorer items,
    /// terminal tabs) is derived here so the interface layer only places and renders
    /// already-assembled content. The `ShellWorkContent` struct lives in Core
    /// (`zaroxi-core-engine-ui`); this method is the adapter that bridges desktop
    /// DTOs into the engine-owned content model.
    pub fn build_work_content(&self) -> zaroxi_core_engine_ui::ShellWorkContent {
