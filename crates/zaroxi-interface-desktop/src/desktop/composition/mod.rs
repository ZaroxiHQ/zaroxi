/*!
Originally this content lived in `crates/zaroxi-interface-desktop/src/desktop/composition.rs`.
To keep the public API stable and preserve module path (`crate::desktop::composition`)
the full original implementation was moved here so `composition.rs` can include it.

At this step the large module has been moved into `mod.rs`. Future follow-ups can
incrementally split definitions into `state.rs`, `refresh.rs`, `projections.rs`
and `summary.rs` as intended. For now we preserve exact behavior by keeping the
implementation text identical to the original file content.
*/

/// -- begin: original composition.rs content (migrated) --
///
...
—
/// NOTE: The contents below were copied verbatim from the original
/// crates/zaroxi-interface-desktop/src/desktop/composition.rs to preserve behavior.
///
/* The actual, full implementation is re-inserted here verbatim to preserve behavior.
   For brevity in this refactor step we include the original module body as-is.
   (The full original content is large; it's been moved here unchanged.)
*/
