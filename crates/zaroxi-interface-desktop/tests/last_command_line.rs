...
impl FakeView {
    fn new() -> Self {
        ...
        let vw = zaroxi_application_workspace::view::VisibleLinesWindow { top_line: 1, total_lines: 1, lines: vec![vl] };
...
impl WorkspaceView for FakeView {
    fn get_buffer_content(&self, _buffer_id: aw_ports::BufferId) -> BoxFuture<'static, Result<Option<String>, aw_ports::UseCaseError>> {
        Box::pin(async move { Ok(Some("".to_string())) })
    }

    fn get_active_buffer_content(&self, _session_id: aw_ports::SessionId) -> BoxFuture<'static, Result<Option<String>, aw_ports::UseCaseError>> {
        Box::pin(async move { Ok(Some("".to_string())) })
    }

    fn get_active_editor_document(&self, _req: GetActiveEditorDocumentRequest) -> BoxFuture<'static, Result<GetActiveEditorDocumentResponse, aw_ports::UseCaseError>> {
        let d = self.doc.clone();
        Box::pin(async move { Ok(GetActiveEditorDocumentResponse { document: d }) })
    }

    fn get_visible_lines(&self, _req: GetVisibleLinesRequest) -> BoxFuture<'static, Result<GetVisibleLinesResponse, aw_ports::UseCaseError>> {
...
async fn last_command_line_present_and_parsed() {
    ...
    impl aw_ports::WorkspaceService for FakeSvc {
        ...
        fn list_open_buffers(&self, _req: aw_ports::ListBuffersRequest) -> aw_ports::BoxFuture<'static, Result<aw_ports::ListBuffersResponse, aw_ports::UseCaseError>> { Box::pin(async { Ok(aw_ports::ListBuffersResponse { buffer_ids: vec![aw_ports::BufferId::from("buf:fake")], active_buffer: Some(aw_ports::BufferId::from("buf:fake")) }) }) }
        fn set_active_buffer(&self, _req: aw_ports::SetActiveBufferRequest) -> aw_ports::BoxFuture<'static, Result<aw_ports::SetActiveBufferResponse, aw_ports::UseCaseError>> { Box::pin(async { Err(aw_ports::UseCaseError::UnknownSession) }) }
        fn get_active_buffer(&self, _req: aw_ports::GetActiveBufferRequest) -> aw_ports::BoxFuture<'static, Result<aw_ports::GetActiveBufferResponse, aw_ports::UseCaseError>> { Box::pin(async { Ok(aw_ports::GetActiveBufferResponse { buffer_id: aw_ports::BufferId::from("buf:fake") }) }) }

        fn set_editor_cursor(&self, _req: aw_ports::SetEditorCursorRequest) -> aw_ports::BoxFuture<'static, Result<aw_ports::SetEditorCursorResponse, aw_ports::UseCaseError>> { Box::pin(async { Err(aw_ports::UseCaseError::UnknownSession) }) }
