...
/// Fake service that returns a recent ExplainExecuted event for the session.
struct FakeSvc {
...
impl FakeSvc {
    fn new(buf: aw_ports::BufferId, wid: zaroxi_kernel_types::Id) -> Self {
        Self { buf, wid }
    }
}
...
impl aw_ports::WorkspaceService for FakeSvc {
    ...
    fn list_open_buffers(&self, _req: aw_ports::ListBuffersRequest) -> aw_ports::BoxFuture<'static, Result<aw_ports::ListBuffersResponse, aw_ports::UseCaseError>> {
        ...
        Box::pin(async move { Ok(aw_ports::ListBuffersResponse { buffer_ids: vec![b], active_buffer: Some(aw_ports::BufferId::from("buf:fake")) }) })
    }
    ...
    fn get_recent_events(&self, req: aw_ports::GetRecentEventsRequest) -> aw_ports::BoxFuture<'static, Result<aw_ports::GetRecentEventsResponse, aw_ports::UseCaseError>> {
        ...
        Box::pin(async move {
            let ev = aw_ports::WorkspaceEvent {
                ...
                workspace_id: wid,
                kind: aw_ports::WorkspaceEventKind::ExplainExecuted { buffer_id: buf.clone(), result: "mocked explain".to_string() },
            };
            Ok(aw_ports::GetRecentEventsResponse { events: vec![ev] })
        })
    }
...
#[tokio::test]
async fn ai_projection_summary_present_and_parsed() {
    let v = FakeView::new();
    let arc: Arc<dyn WorkspaceView> = Arc::new(v);
    let sid = SessionId(Id::new());
    let wid = Id::new();

    let fake_service = std::sync::Arc::new(FakeSvc::new(aw_ports::BufferId::from("buf:fake"), wid.clone())) as std::sync::Arc<dyn aw_ports::WorkspaceService>;
    ...
    let sum = summary_opt.unwrap();
    assert!(sum.present);
    assert_eq!(sum.kind.unwrap(), AiKind::Explain);
    assert_eq!(sum.target_buffer.unwrap(), aw_ports::BufferId::from("buf:fake"));
    assert_eq!(sum.state, AiState::Ready);
}

#[tokio::test]
async fn ai_projection_summary_absent_when_no_events() {
    let v = FakeView::new();
    let arc: Arc<dyn WorkspaceView> = Arc::new(v);
    let sid = SessionId(Id::new());
    let wid = Id::new();
