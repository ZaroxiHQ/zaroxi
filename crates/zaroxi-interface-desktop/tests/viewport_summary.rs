...
impl WorkspaceView for FakeView {
    ...
    ) -> aw_ports::BoxFuture<'static, Result<GetActiveEditorDocumentResponse, aw_ports::UseCaseError>>
    {
    ...
    ) -> aw_ports::BoxFuture<'static, Result<GetVisibleLinesResponse, aw_ports::UseCaseError>> {
...
async fn initial_refresh_populates_viewport_summary() {
    ...
    let ar = zaroxi_interface_desktop::refresh_desktop(&mut comp, arc, sid.clone(), None, None)
        .await
        .expect("refresh ok");
    assert!(ar.success);
...
async fn move_cursor_action_updates_viewport_summary() {
    ...
    impl FakeSvc {
        fn new(buf: aw_ports::BufferId) -> Self {
            Self { buf }
        }
    }
    impl aw_ports::WorkspaceService for FakeSvc {
        ...
        > {
            Box::pin(async { Ok(aw_ports::SetActiveBufferResponse { ok: true }) })
        }
        ...
        > {
            ...
            Box::pin(async move { Ok(aw_ports::GetActiveBufferResponse { buffer_id: b }) })
        }
        ...
        > {
            Box::pin(async { Ok(aw_ports::SetEditorCursorResponse { ok: true }) })
        }
    ...
    let service = std::sync::Arc::new(FakeSvc::new(aw_ports::BufferId::from("buf:fake")))
        as std::sync::Arc<dyn aw_ports::WorkspaceService>;

    // Pre-refresh to populate the presenter
    let _ =
        zaroxi_interface_desktop::refresh_desktop(&mut comp, arc.clone(), sid.clone(), None, None)
            .await
            .expect("initial refresh ok");

    // Execute the move-cursor action which will call set_editor_cursor and refresh composition.
    let res = zaroxi_interface_desktop::move_cursor_to_start_and_refresh(
        &mut comp,
        service.clone(),
        arc.clone(),
        sid.clone(),
    ...
    assert!(matches!(
        vs.anchoring,
        ViewportAnchoring::Top | ViewportAnchoring::Centered | ViewportAnchoring::Unknown
    ));
}
...
async fn content_mutation_reflects_in_viewport_total_lines() {
    ...
    impl MutableFakeView {
        fn new(buffer_id: BufferId, content: Option<String>, cursor: EditorCursor) -> Self {
            ...
            Self {
                ...
                window: std::sync::Arc::new(std::sync::Mutex::new(vw)),
            }
        }
    ...
    impl WorkspaceView for MutableFakeView {
        ...
        fn get_active_editor_document(
            ...
            'static,
            Result<GetActiveEditorDocumentResponse, aw_ports::UseCaseError>,
        > {
    ...
    let _ =
        zaroxi_interface_desktop::refresh_desktop(&mut comp, arc2.clone(), sid.clone(), None, None)
            .await
            .expect("refresh ok");
