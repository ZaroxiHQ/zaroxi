...
use zaroxi_application_workspace::ports::{
    EditorCursor, GetEditorStateRequest, OpenBufferRequest, Selection, SetEditorCursorRequest,
...
async fn set_and_get_cursor_and_selection() {
    ...
    // Set selection
    let sel = Selection {
        anchor: EditorCursor { line: 1, column: 0 },
        active: EditorCursor { line: 1, column: 10 },
    };
    let sel_res = orchestrator
    ...
    // Clear selection
    let clear_res = orchestrator
        .clear_editor_selection(crate::ports::ClearSelectionRequest {
            session_id: boot_res.session.session_id.clone(),
            buffer_id: open_res.buffer_id.clone(),
        })
        .await
