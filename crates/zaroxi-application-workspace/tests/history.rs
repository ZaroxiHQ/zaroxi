...
use zaroxi_application_workspace::ports::{
    GetRecentCommandsRequest, GetRecentEventsRequest, OpenBufferRequest, WorkspaceBootRequest,
};
...
async fn history_and_events_recorded() {
    ...
    let cmds = orchestrator
        ...
        .expect("query cmds");
    assert!(
        cmds.commands.iter().any(|c| matches!(c.kind, ports::CommandKind::BootWorkspace { .. }))
    );
    assert!(cmds.commands.iter().any(|c| matches!(c.kind, ports::CommandKind::OpenBuffer { .. })));

    let evs = orchestrator
    ...
    assert!(
        evs.events
            .iter()
            .any(|e| matches!(e.kind, ports::WorkspaceEventKind::SessionOpened { .. }))
    );
    assert!(
        evs.events.iter().any(|e| matches!(e.kind, ports::WorkspaceEventKind::BufferOpened { .. }))
    );
}
