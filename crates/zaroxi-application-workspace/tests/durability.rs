...
use zaroxi_application_workspace::ports::{
    LoadCheckpointRequest, SaveCheckpointRequest, WorkspaceBootRequest,
};
...
async fn save_and_load_roundtrip() {
    ...
    let history = Arc::new(zaroxi_infrastructure_memory::InMemoryHistoryStore::new())
        as Arc<dyn ports::HistoryRepository>;
    ...
    let open = ports::OpenBufferRequest {
    ...
    let history2 = Arc::new(zaroxi_infrastructure_memory::InMemoryHistoryStore::new())
        as Arc<dyn ports::HistoryRepository>;
    ...
    let snap = orchestrator2
        .get_session_snapshot(ports::GetSessionSnapshotRequest {
...
async fn malformed_data_fails() {
    ...
    let err = orchestrator
        .load_checkpoint(LoadCheckpointRequest { location: bad_id.clone() })
        .await
        .expect_err("should fail");
    assert!(
        err.to_string().contains("invalid checkpoint") || err.to_string().contains("malformed")
    );
}
