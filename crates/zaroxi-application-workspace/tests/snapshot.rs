...
use zaroxi_application_workspace::ports::{
    GetSessionSnapshotRequest, OpenBufferRequest, SetActiveBufferRequest, UpdateBufferRequest,
...
async fn snapshot_reflects_active_change_and_mutation() {
    ...
    let main_snapshot =
        snap.buffers.iter().find(|bs| bs.buffer_id == open_res.buffer_id).expect("main present");
