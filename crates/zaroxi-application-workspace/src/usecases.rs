...
use crate::ports::{
    ...
    EditorCursor,
    EditorState,
    GetActiveBufferRequest,
    GetActiveBufferResponse,
    GetEditorStateRequest,
    GetEditorStateResponse,
    GetRecentCommandsRequest,
    ...
    ScrollViewportResponse,
    SessionId,
    SessionSnapshot,
    SetActiveBufferRequest,
    SetActiveBufferResponse,
    SetEditorCursorRequest,
    SetEditorCursorResponse,
    SetSelectionRequest,
...
fn apply_text_transaction_impl(
    ...
    Box::pin(async move {
        ...
        // Ensure buffer is opened in session and prepare editor-state mutation
        {
            ...
            let is_open = {
                let s = sessions.lock().unwrap();
                s.get(&req.session_id.0)
                    .map(|info| info.open_buffers.iter().any(|b| b == &req.buffer_id))
                    .unwrap_or(false)
            };
            ...
            let entry = info
                .editor_states
                .entry(req.buffer_id.clone())
                .or_insert(EditorState { cursor: EditorCursor::zero(), selection: None });
            ...
            match &req.transaction {
                TextEdit::Insert { index, text } => {
                    if entry.cursor.line == 0 && entry.cursor.column as usize == *index {
                        new_cursor.column =
                            new_cursor.column.saturating_add(text.chars().count() as u32);
                    }
                ...
                TextEdit::Replace { start, end, text } => {
                    ...
                    if entry.cursor.line == 0 {
                        ...
                        if cidx >= *start && cidx <= *end {
                            ...
                            if new_len >= old_len {
                                new_cursor.column =
                                    new_cursor.column.saturating_add((new_len - old_len) as u32);
                            } else {
                                new_cursor.column =
                                    new_cursor.column.saturating_sub((old_len - new_len) as u32);
                            }
                        }
                    }
                    entry.selection = None;
        ...
        let state = {
            let s = sessions.lock().unwrap();
            s.get(&req.session_id.0)
                .and_then(|info| info.editor_states.get(&req.buffer_id).cloned())
        };

        Ok(ApplyTextTransactionResponse {
            ok: true,
            state: state.unwrap_or(EditorState { cursor: EditorCursor::zero(), selection: None }),
            content,
        })
    })
}
...
impl crate::ports::DurabilityRepository for NoopDurability {
    fn save_checkpoint(
        &self,
        _checkpoint: crate::ports::Checkpoint,
    ) -> BoxFuture<'static, Result<String, crate::ports::DurabilityError>> {
        Box::pin(async {
            Err(crate::ports::DurabilityError::Io("noop durability not configured".to_string()))
        })
    }
    fn load_checkpoint(
        &self,
        _location: String,
    ) -> BoxFuture<'static, Result<crate::ports::Checkpoint, crate::ports::DurabilityError>> {
        Box::pin(async {
            Err(crate::ports::DurabilityError::NotFound(
                "noop durability not configured".to_string(),
            ))
        })
    }
}
...
impl crate::ports::WorkspaceService for WorkspaceOrchestrator {
    fn boot_workspace(
        ...
        Box::pin(async move {
            ...
            let dto = repo
                .open_workspace(domain_cmd)
                .await
                .map_err(|_e| UseCaseError::UnknownWorkspace)?;
            ...
            let session = WorkspaceSessionDTO {
                session_id: crate::ports::SessionId(session_id),
                workspace_id: dto.id,
            };
            ...
            let ev = WorkspaceEvent {
                ...
                workspace_id: dto.id,
                kind: WorkspaceEventKind::SessionOpened {
                    session_id: session.session_id.clone(),
                    workspace_id: dto.id,
                },
            };
        ...
        Box::pin(async move {
            ...
            if let Some(ws) = workspace_id_opt {
                let ev = WorkspaceEvent {
                    ...
                    session_id: req.session_id.clone(),
                    workspace_id: ws,
                    kind: WorkspaceEventKind::BufferOpened {
    ...
    fn list_open_buffers(
        ...
        Box::pin(async move {
            ...
            Ok(ListBuffersResponse {
                buffer_ids: info.open_buffers.clone(),
                active_buffer: info.active_buffer.clone(),
            })
        })
    }
    ...
    fn set_active_buffer(
        ...
        Box::pin(async move {
            ...
            let ev = WorkspaceEvent {
                ...
                session_id: req.session_id.clone(),
                workspace_id,
                kind: WorkspaceEventKind::ActiveBufferChanged {
                    old,
                    new: Some(req.buffer_id.clone()),
                },
            };
            let _ = history.record_event(ev).await;
        ...
        Box::pin(async move {
            ...
            if content.trim().is_empty() {
                ...
                return Err(UseCaseError::AiFailure(
                    "missing buffer content for explain".to_string(),
                ));
            }
            ...
            let ev = WorkspaceEvent {
                ...
                session_id: req.session_id.clone(),
                workspace_id,
                kind: WorkspaceEventKind::ExplainExecuted {
                    buffer_id: active.clone(),
                    result: res.text.clone(),
                },
            };
            let _ = history.record_event(ev).await;
            ...
            match req.command {
                    ...
                    if content.trim().is_empty() {
                        ...
                        let cmd = CommandRecord::new_failure(
                            CommandKind::DispatchAppCommand {
                                command: AppCommand::AiExplain { buffer_id: buffer_id.clone() },
                            },
                            Some(req.session_id.0),
                            Some(workspace_id),
                            Some(buffer_id.clone()),
                ...
                AppCommand::InsertText { buffer_id, text } => {
                    ...
                    let is_open = {
                        let s = sessions.lock().unwrap();
                        s.get(&req.session_id.0)
                            .map(|info| info.open_buffers.iter().any(|b| b == &buffer_id))
                            .unwrap_or(false)
                    };
                    if !is_open {
                        let cmd = CommandRecord::new_failure(
                    ...
                    let cursor = {
                        let s = sessions.lock().unwrap();
                        s.get(&req.session_id.0)
                            .and_then(|info| info.editor_states.get(&buffer_id).cloned())
                    }
                    .unwrap_or(EditorState { cursor: EditorCursor::zero(), selection: None })
                    .cursor;
                    ...
                    match apply_text_transaction_impl(
                        store.clone(),
                        sessions.clone(),
                        history.clone(),
                        txn_req,
                ...
                AppCommand::DeleteSelection { buffer_id } => {
                    ...
                    if !is_open {
                        let cmd = CommandRecord::new_failure(
                            CommandKind::DispatchAppCommand {
                                command: AppCommand::DeleteSelection {
                                    buffer_id: buffer_id.clone(),
                                },
                            },
                            Some(req.session_id.0),
                    ...
                    let selection_opt = {
                        let s = sessions.lock().unwrap();
                        s.get(&req.session_id.0).and_then(|info| {
                            info.editor_states.get(&buffer_id).and_then(|st| st.selection.clone())
                        })
                    };
                    ...
                    .await
                    {
                        Ok(_) => {
                            ...
                            let _ = history.record_command(cmd).await;
                            Ok(DispatchCommandResponse {
                                result: CommandResult { message: "deleted".to_string() },
                            })
                        }
                        Err(e) => {
                            let cmd = CommandRecord::new_failure(
                    ...
                    if let Some(selection) = selection_opt {
                        ...
                        .await
                        {
                            Ok(_) => {
                                ...
                                let _ = history.record_command(cmd).await;
                                Ok(DispatchCommandResponse {
                                    result: CommandResult { message: "replaced".to_string() },
                                })
                            }
                            Err(e) => {
                                let cmd = CommandRecord::new_failure(
                ...
                AppCommand::IndentLine { buffer_id } => {
                    ...
                    if !is_open {
                        let cmd = CommandRecord::new_failure(
                            CommandKind::DispatchAppCommand {
                                command: AppCommand::IndentLine { buffer_id: buffer_id.clone() },
                            },
                            Some(req.session_id.0),
                            Some(workspace_id),
                            Some(buffer_id.clone()),
                    ...
                    let txn_req = ApplyTextTransactionRequest {
                        session_id: req.session_id.clone(),
                        buffer_id: buffer_id.clone(),
                        transaction: TextEdit::Insert {
                            index: line_start_chars,
                            text: "    ".to_string(),
                        },
                    };
                    ...
                    .await
                    {
                        Ok(_) => {
                            ...
                            let _ = history.record_command(cmd).await;
                            Ok(DispatchCommandResponse {
                                result: CommandResult { message: "indented".to_string() },
                            })
                        }
                        Err(e) => {
                            let cmd = CommandRecord::new_failure(
                    ...
                    TextEdit::Replace { start, end, text } => {
                        ...
                        if entry.cursor.line == 0 {
                            ...
                            if cidx >= *start && cidx <= *end {
                                ...
                                if new_len >= old_len {
                                    new_cursor.column = new_cursor
                                        .column
                                        .saturating_add((new_len - old_len) as u32);
                                } else {
                                    new_cursor.column = new_cursor
                                        .column
                                        .saturating_sub((old_len - new_len) as u32);
                                }
                            }
                        }
                        entry.selection = None;
    ...
    fn get_recent_commands(
        ...
        let history = self.history.clone();
        Box::pin(async move {
            let recs: Vec<CommandRecord> = history
                .get_recent_commands(req.session_id.clone(), req.limit)
                .await
                .map_err(|_e| UseCaseError::AiFailure("history query failed".to_string()))?;
    ...
    fn get_recent_events(
        ...
        let history = self.history.clone();
        Box::pin(async move {
            let evs: Vec<WorkspaceEvent> = history
                .get_recent_events(req.session_id.clone(), req.limit)
                .await
                .map_err(|_e| UseCaseError::AiFailure("history query failed".to_string()))?;
    ...
    fn get_session_snapshot(
        ...
        Box::pin(async move {
            ...
            let commands = history
                .get_recent_commands(req.session_id.clone(), req.recent_limit)
                .await
                .map_err(|_e| UseCaseError::AiFailure("history query failed".to_string()))?;
            let events = history
                .get_recent_events(req.session_id.clone(), req.recent_limit)
                .await
                .map_err(|_e| UseCaseError::AiFailure("history query failed".to_string()))?;
    ...
    fn create_checkpoint(
        ...
        Box::pin(async move {
            ...
            // Recent commands and events (read from history port).
            let commands = history
                .get_recent_commands(req.session_id.clone(), 50)
                .await
                .map_err(|_e| UseCaseError::AiFailure("history query failed".to_string()))?;
            let events = history
                .get_recent_events(req.session_id.clone(), 50)
                .await
                .map_err(|_e| UseCaseError::AiFailure("history query failed".to_string()))?;
    ...
    fn save_checkpoint(
        &self,
        req: crate::ports::SaveCheckpointRequest,
    ) -> BoxFuture<'static, Result<crate::ports::SaveCheckpointResponse, UseCaseError>> {
        let store = self.buffer_store.clone();
        let sessions = self.sessions.clone();
        let history = self.history.clone();
    ...
    fn load_checkpoint(
        &self,
        req: crate::ports::LoadCheckpointRequest,
    ) -> BoxFuture<'static, Result<crate::ports::LoadCheckpointResponse, UseCaseError>> {
        let store = self.buffer_store.clone();
        let sessions = self.sessions.clone();
        let history = self.history.clone();
        let durability = self.durability.clone();
        Box::pin(async move {
            ...
            let ck = match durability.load_checkpoint(req.location.clone()).await {
                ...
                Err(e) => {
                    ...
                    match e {
                        crate::ports::DurabilityError::Malformed(s) => {
                            return Err(UseCaseError::InvalidCheckpoint(s));
                        }
                        crate::ports::DurabilityError::UnknownVersion(v) => {
            ...
            // Insert session info
            {
                ...
                s.insert(
                    ...
                    SessionInfo {
                        ...
                        open_buffers: ck.opened_buffers.clone(),
            ...
            let session = WorkspaceSessionDTO {
                session_id: ck.session_id.clone(),
                workspace_id: ck.workspace_id,
            };
...
impl crate::ports::WorkspaceView for WorkspaceOrchestrator {
    ...
    ) -> BoxFuture<'static, Result<Option<String>, crate::ports::UseCaseError>> {
        let store = self.buffer_store.clone();
        // synchronous read path from BufferStore
        Box::pin(async move { Ok(store.get_text(&buffer_id)) })
    }
    ...
    fn get_active_editor_document(
        &self,
        req: crate::ports::GetActiveEditorDocumentRequest,
    ) -> BoxFuture<
        'static,
    ...
    > {
        ...
        Box::pin(async move {
            ...
            let state =
                info.editor_states.get(&active).cloned().unwrap_or(crate::ports::EditorState {
            ...
            let current_line = content
                .as_ref()
                .and_then(|c| c.lines().nth(state.cursor.line as usize).map(|s| s.to_string()));
    ...
    fn get_visible_lines(
        &self,
        req: crate::ports::GetVisibleLinesRequest,
    ) -> BoxFuture<'static, Result<crate::ports::GetVisibleLinesResponse, crate::ports::UseCaseError>>
    {
        let sessions = self.sessions.clone();
        let store = self.buffer_store.clone();
        Box::pin(async move {
            ...
            let info = {
                ...
                s.get(&req.session_id.0)
                    .cloned()
                    .ok_or(crate::ports::UseCaseError::UnknownSession)?
            };

            // Ensure buffer is opened in session
            if !info.open_buffers.iter().any(|b| b == &req.buffer_id) {
                return Err(crate::ports::UseCaseError::InvalidActiveBuffer(
                    req.buffer_id.to_string(),
                ));
            }

            // Snapshot content
            let content = store.get_text(&req.buffer_id);
            let state = info.editor_states.get(&req.buffer_id).cloned().unwrap_or(
                crate::ports::EditorState {
                    cursor: crate::ports::EditorCursor::zero(),
                    selection: None,
                },
            );
