...
MIN_RUST_VERSION = "1.76.0"      # Minimum supported Rust version (MSRV) for edition 2024 compatibility
CRATE_VERSION = "0.1.0"
LICENSE = "MIT"
...
def crate_description(crate_name: str) -> str:
    ...
    This function produces a clear, publication-ready description for every crate.
    """
    # If a future need arises to override a specific crate description, add it here.
    ...
    if crate_name in overrides:
        return overrides[crate_name]

    # Heuristic generation: convert crate name into a clean human sentence.
    # Examples:
    #   zaroxi-core-engine-render -> "Core engine render subsystem for the Zaroxi platform."
    #   zaroxi-domain-workspace -> "Domain workspace subsystem for the Zaroxi platform."
    core = crate_name.replace("zaroxi-", "")
    words = core.split("-")
    # Title like "Core engine render"
    title = " ".join(words).replace("  ", " ").strip().capitalize()
    layer = detect_layer(crate_name)
    return f"{title} subsystem for the Zaroxi platform. Layer: {layer}."
...
def write_crate(crate_name: str):
    ...
    lib_rs = textwrap.dedent(f"""\
        ...
        //!
        //! Generated by tools/create_crates.py (RUST_EDITION={RUST_EDITION}, RUST_TOOLCHAIN={RUST_TOOLCHAIN}, MIN_RUST_VERSION={MIN_RUST_VERSION}).
...
def write_workspace_toml():
    ...
    pins the workspace edition to the mandated RUST_EDITION. Also emits a
    Local workspace crates section that maps crate names to local paths for
    easier consumption by editors and tooling.
    """
    ...
    # Local workspace crates mapping (for editors/tooling that prefer named entries)
    local_crates = "\n".join([f'{c} = {{ path = "crates/{c}" }}' for c in CRATES])
    ...
    workspace_toml = textwrap.dedent(f"""\
        ...
        # Minimum Rust version (MSRV) required for this workspace (for CI/tooling)
        [workspace.metadata]
        min-rust-version = "{MIN_RUST_VERSION}"

        # Local workspace crates (paths correspond to the new architecture)
        {local_crates}
    """)
...
def main():
    ...
    print(' - git commit -m "chore: bootstrap Zaroxi crate skeletons and workspace (edition=2024, msrv={MIN_RUST_VERSION})"')
